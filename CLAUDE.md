...
- `TestSuiteService` — browser-based test recorder and Playwright runner. Records user interactions via WebContentsView preload, generates `.spec.ts` files with smart waits and Playwright-preferred locators (`getByTestId` > `getByLabel` > `getByRole` > `getByText` > CSS fallback), runs tests via `npx playwright test --reporter=json`, tracks per-step pass/fail results in SQLite. Supports scheduling, visual baselines (pixel-diff), data-driven testing (CSV/JSON `{{key}}` substitution), shared step groups, and CI export (GitHub Actions YAML). Config: per-project `TestSuiteConfig` with `navigationTimeout`, `actionTimeout`, viewport, screenshot mode, `browsers` (chromium/firefox/webkit multi-select), `workers` (1-16 parallel), and `environments` (named URL profiles with runtime switching via `BASE_URL` env var). Scripts have `tags` (string array, stored as JSON in SQLite) for categorization and filtering. Library panel supports tag-based intersection filtering. UI: single Zustand store (`test-suite-store.ts`), 7-tab page (Recording, Library, Results, Screenshots, Analytics, Shared Steps, CI Export). Results tab has environment selector, full-output log dialog, Create Task + Start Workflow integration with progress pipeline.
