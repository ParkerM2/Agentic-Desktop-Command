// tests/integration/peers/sync-tables-notes.test.ts
...
describe('sync-tables: notes round-trip', () => {
  ...
  it('A note insert appears on B', async () => {
    sqliteA
      .prepare(
        `INSERT INTO notes (id, title, content, tags, project_id, task_id, pinned, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .run('note-sync-1', 'Hello note', 'Body content', '[]', null, null, 0, '2026-04-24T00:00:00Z', '2026-04-24T00:00:00Z');
    ...
    const row = await waitFor(() =>
      sqliteB.prepare(`SELECT title FROM notes WHERE id='note-sync-1'`).get() as
        | { title: string }
        | undefined,
    );
