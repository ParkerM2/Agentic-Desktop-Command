...
interface RelayJoinRow extends SessionRelayRow {
  claimed_by_device_id: string;
  host_device_id: string;
  project_id: string;
}
...
/**
 * Validate that sourceDeviceId holds an active (unexpired) claim on projectId.
...
function validateClaim(
  ...
  const claim = db
    ...
    .get(projectId, sourceDeviceId, now) as ProjectClaimRow | undefined;
...
/**
 * Check if a raw WS message is a relay message (starts with session.*).
 */
export function isRelayMessage(raw: string): boolean {
  try {
...
function handleSessionSpawn(
  ...
  // Use project_id as the claim_id reference (since project_id is the PK)
  db.prepare(
    `INSERT INTO session_relay (id, claim_id, status, started_at)
     VALUES (?, ?, 'active', ?)`,
  ).run(sessionId, claim.project_id, new Date().toISOString());
