...
| **progress** | SQLite-backed progress tracking (replaces old file-based task system). Sub-modules: `progress-service.ts`, `task-file-io.ts`, `schema.ts` (Drizzle ORM, `progress_tasks` table). All task data stored in SQLite. | listTasks, getTask, createTask, updateTask, deleteTask | `event:progress.*` |
