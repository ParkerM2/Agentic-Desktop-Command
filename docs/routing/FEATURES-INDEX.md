...
| **assistant** | Global Claude assistant (single instance, app lifetime) | AssistantWidget, SidebarAssistantButton (inline chat + popup), AssistantInputBar (ProjectSelector + QuickActionChips), WidgetPanel, WidgetMessageArea; **Backend**: tool-definitions.ts (system prompt + tool schemas), tool-executor.ts (routes tool_use to services), tool-handlers/ (task-tools, git-tools, project-tools) | `assistant.start`, `assistant.sendCommand`, `assistant.getHistory`, `assistant.clearHistory`, `event:assistant.response`, `event:assistant.thinking`, `event:assistant.toolExecuted` |
...
| **notes** | Note-taking *(accessed via Productivity > Notes tab)* | NotesPage, NoteEditor, NoteList | `notes.*` |
| **planner** | Daily planner *(accessed via Productivity > Planner tab)* | PlannerPage, TimeBlockGrid, TimeBlockCard | `planner.*` |
...
| **tasks** | Task management (TanStack Table) | ProgressTaskGrid (TanStack Table + `@ui` Table primitives in `<Card>`), TaskFiltersToolbar, ProgressTaskDetailRow, TaskStatusBadge, CreateTaskDialog, PlanFeedbackDialog, TaskResultView, CreatePrDialog; **Hooks**: useTaskEvents, useAgentMutations, useQaMutations, QaReportViewer | `progress.*`, `tasks.*`, `agent.*`, `qa.*`, `git.createPr`, `event:bus.*`, `event:qa.*` |
...
| **tasks (progress)** | SQLite-backed task pipeline grid (all data in `progress_tasks` table — SQLite is sole source of truth). Reads from React Query hooks (`useProgress`, `useProgressMutations`) with EventBridge-driven invalidation. No optimistic updates (IPC <1ms). Columns: status, title, priority, stage (research/plan/team indicators), Jira badge, PR badge, updated, done stage badge, completed priority badge. Expanded row shows Research/Plan/Team pipeline with per-step action buttons and live agent activity. | ProgressTaskGrid, ProgressTaskDetailRow, TeamActivityPanel, AgentDetailExpander | `progress.*` |
...
|---------|---------|-------------|----------------|
| **agent** | Claude CLI process management (legacy v1). Sub-modules: `agent-spawner.ts`, `agent-output-parser.ts`, `agent-queue.ts`, `token-parser.ts` | spawnAgent, stopAgent, pauseAgent, resumeAgent | `event:agent.*` |
...
| **project** | Project management. Sub-modules: `project-detector.ts`, `codebase-analyzer.ts` (tech stack detection: languages, frameworks, package managers, build tools, test frameworks, linters, TypeScript, Tailwind, Node version, monorepo tools — sync, max 3 levels deep) | list, add, remove, selectDirectory, analyzeCodebase | `event:project.*` |
...
| **progress (handlers)** | IPC handlers for ProgressService. Exposes all task pipeline operations and agent session spawning via `PROGRESS.*` channel constants. | listTasks, getTask, createTask, updateTask, archiveTask, deleteTask, listArchived, startResearch, createPlan, spinUpTeam, runWorkflow, cancelAction | `event:progress.task.updated`, `event:progress.task.created`, `event:progress.task.archived`, `event:progress.action.started`, `event:progress.action.completed`, `event:progress.action.failed`, `event:progress.workflow.step` |
| **agent-manager** | Headless Claude process spawning via stream-json protocol. Now runs inside the Agent Host utility process (`src/main/agent-host/`). Main process accesses via `AgentHostClient` (MessagePort RPC proxy). Both implement the shared `AgentManager` interface. Direct MessagePort to renderer for stream events. Sub-modules: `process-manager.ts`, `stream-json-parser.ts`, `agent-connection-strategy.ts`, `subprocess-strategy.ts` | spawnProjectOwner, spawnTeamLead, listSessions, getSession, sendMessage, stopSession | `event:agent-dashboard.*` |
...
| `task-handlers.ts` | tasks.* (legacy Hub task handlers — `HUB_TASKS` handlers deleted; remaining handlers route through ProgressService) |
